...
function compute_url(){
    ...
    # NOTE Qt6: Support for qtwebengine extension.
        ...
        if [[ "${VERSION}" > "$(version 6.6.0)" ]] && [[ "${TARGET_PLATFORM}" == "android" ]]; then
...
for COMPONENT in ${COMPONENTS}; do
    ...
    if [[ "${TOOLCHAIN}" =~ "win64_mingw" ]]; then
        ...
        SUBDIR="android"
    elif [[ "${HOST_OS}" == "mac_x64" ]] && [[ ! "${VERSION}" < "$(version 6.1.2)" ]] && [[ "${TARGET_PLATFORM}" == "desktop" ]]; then
        SUBDIR="macos"
    else
        SUBDIR="${TOOLCHAIN}"
    fi
    ...
    elif [ "${TARGET_PLATFORM}" == "ios" ] && [ ! "${VERSION}" \< "$(version 6.0.0)" ]; then
        CONF_FILE="${UNPACK_DIR}/${VERSION}/${SUBDIR}/bin/target_qt.conf"
    ...
    elif [ "${TARGET_PLATFORM}" == "wasm" ] && [ ! "${VERSION}" \< "$(version 6.0.0)" ]; then
        CONF_FILE="${UNPACK_DIR}/${VERSION}/${SUBDIR}/bin/target_qt.conf"
        sed -i.bak "s|HostData=target|HostData=../$TOOLCHAIN|g" "${CONF_FILE}"

...
function compute_url(){
    ...
    # NOTE Qt6: Support for qtwebengine extension.
        ...
        if [[ "$VERSION_NUMBER" > "$(version 6.6.0)" ]] && [[ "${TARGET_PLATFORM}" == "android" ]]; then
...
for COMPONENT in ${COMPONENTS}; do
    ...
    if [ "$VERSION_NUMBER" -ge "$(version "6.0.0")" ]; then
    ...
    if [[ "${TOOLCHAIN}" =~ "win64_mingw" ]]; then
        ...
        SUBDIR="android"
    elif [[ "${HOST_OS}" == "mac_x64" ]] && [[ ! "$VERSION_NUMBER" < "$(version 6.1.2)" ]] && [[ "${TARGET_PLATFORM}" == "desktop" ]]; then
        SUBDIR="macos"
    else
        SUBDIR="${TOOLCHAIN}"
    fi
    ...
    if [[ "$VERSION_NUMBER" > "$(version 6.6.0)" ]]; then
    ...
    if [ "${TARGET_PLATFORM}" == "android" ] && [ ! "$VERSION_NUMBER" \< "$(version 6.0.0)" ]; then
        CONF_FILE="${UNPACK_DIR}/${VERSION}/${SUBDIR}/bin/target_qt.conf"
        ANDROID_QMAKE_FILE="${UNPACK_DIR}/${VERSION}/${SUBDIR}/bin/qmake"
        if [ "${TOOLCHAIN}" == "android_armv7" ] && [ ! "$VERSION_NUMBER" \< "$(version 6.4.2)" ]; then
            sed -i "s/\r//" "${CONF_FILE}"
    ...
    elif [ "${TARGET_PLATFORM}" == "ios" ] && [ ! "$VERSION_NUMBER" \< "$(version 6.0.0)" ]; then
        CONF_FILE="${UNPACK_DIR}/${VERSION}/${SUBDIR}/bin/target_qt.conf"
    ...
    elif [ "${TARGET_PLATFORM}" == "wasm" ] && [ ! "$VERSION_NUMBER" \< "$(version 6.0.0)" ]; then
        CONF_FILE="${UNPACK_DIR}/${VERSION}/${SUBDIR}/bin/target_qt.conf"
        sed -i.bak "s|HostData=target|HostData=../$TOOLCHAIN|g" "${CONF_FILE}"
